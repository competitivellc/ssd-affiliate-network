...
function hostPortWarning(iface: string): string {
  if (iface === "USB4" || iface === "Thunderbolt 4" || iface === "Thunderbolt 3") {
    return pick([
      ...
      "At these speeds, enclosure material matters — anodized aluminum helps passively dissipate the heat generated by sustained 40 Gbps transfers, while plastic enclosures may throttle sooner.",
    ]);
  }
...
function interfaceBandwidthNote(iface: string): string {
  if (iface === "USB4" || iface === "Thunderbolt 4") {
    return pick([
      ...
      "NVMe drives over Thunderbolt 4 or USB4 unlock the full potential of the NAND — unlike SATA-based externals that bottleneck at 550 MB/s regardless of the host port.",
    ]);
  }
  if (iface === "USB 3.2 Gen 2x2") {
    return pick([
      ...
      "Gen 2x2 is a true NVMe pipe — unlike SATA-based USB drives that top out around 550 MB/s, this interface lets the drive's controller stretch its legs.",
    ]);
  }
  return pick([
    ...
    "Over 10 Gbps USB, an NVMe-based external still outperforms a SATA-based one — expect 800-1000 MB/s real-world reads vs. 550 MB/s for SATA, especially with mixed workloads.",
  ]);
}
...
function enduranceNotice(tbw: number): string {
  if (tbw >= 1000) {
    return pick([
      ...
      `${tbw.toLocaleString()} TBW lifespan durability means it survives years of rewrites — think daily 4K video transfers, database backups, or OS installations without wearing out.`,
    ]);
  }
  if (tbw >= 500) {
    return pick([
      ...
      `${tbw.toLocaleString()} TBW lifespan durability is a solid mid-range rating — enough for regular use with room to spare, and likely to outlast your laptop.`,
    ]);
  }
  return pick([
    ...
    `${tbw.toLocaleString()} TBW lifespan durability means this drive is best for occasional backups and file transfers — if you rewrite the full capacity daily, expect it to wear out sooner than premium alternatives.`,
  ]);
}
...
export function generateUseCaseEditorial(
  ...
  if (isSlow) {
    conditionalBlocks.push({
      ...
      content: pick([
        ...
        "The SATA ceiling of ~550 MB/s means these drives top out well below what NVMe-based externals deliver — fine for documents and backups, but expect longer waits on large media files.",
        "Most drives in this tier use SATA-based controllers rather than NVMe, which caps real-world throughput around 550 MB/s. If you frequently transfer 4K footage or large datasets, an NVMe external will save significant time.",
      ]),
    });
  }
  ...
  const possibleBullets = [
    ...
    `${top?.tbw?.toLocaleString() || "N/A"} TBW lifespan durability rating for long-term reliability`,
    "AES 256-bit hardware encryption for data security on the go",
    "Aluminum enclosure construction for passive heat dissipation during sustained transfers",
  ];
  ...
  if (isSlow) {
    conditionalBlocks.push({
      ...
      content: pick([
        ...
        "SATA-based controller architecture explains the sub-1 GB/s ceiling here — NVMe-based externals typically deliver 2-5x the throughput for a modest price premium.",
        "Unlike NVMe drives that pipe through multiple PCIe lanes, these SATA-based externals are limited by the AHCI protocol. Fine for everyday use, but a bottleneck for creative pros.",
      ]),
    });
  }
