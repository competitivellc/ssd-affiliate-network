...
**Resolved (2026-08-03):** the GA4 issue is now closed. Properties are confirmed wired correctly (`scripts/ga4_audit_properties.js` shows both property IDs match their web streams with the correct measurement IDs, both service accounts have Viewer access). A live `node scripts/ga4_pull_traffic.js` run on 2026-08-03 returned real data for `externalssds.com` (5 sessions, 1 user, 6 pageviews, 1 engaged session over the 28-day window — top page `/products/samsung-t9-4tb`). `portablessds.com` still returns zero rows at the time of audit, which is expected: the property is brand-new, traffic is single-digit monthly, and the cookie-consent banner gates gtag loading. Once traffic materially grows and users accept cookies, portablessds data will populate naturally — no further code action is required.
