...
> ## 🚨 START HERE — Measure gates for 01ef4ab (2026-09-01 portablessds /compare bing-ctr) — NEXT AI MUST RUN — THIS IS STEP 1 OF NEXT PULLS, ALL STEPS MUST REMAIN HERE UNTIL DONE
> **Any AI opening this repo must run (no code changes until gates close):** `node scripts/bing_pull_traffic.js` + `node scripts/gsc_pull_revenue.js --force` + `node scripts/ga4_pull_traffic.js` + RUM GraphQL (`requestHost=portablessds.com`/`externalssds.com` via `SSD_CLOUDFLARE_WEB_ANALYTICS_API_TOKEN` — RUM = real pageloads, GSC impressions = SERP renders only). Also run `node scripts/affiliate_clicks_report.js` if D1 auth fixed (`7403` until `npx wrangler login` full scope).
> - **Baseline 2026-09-01 (Day 0, commit `01ef4ab` live):** Bing portablessds `312i 5c 1.60%` (`portable ssds comparison` `31i+15i+14i` `0c`, `buying guide` `15i+10i` `0c`); GSC portablessds `12c/2622i pos35.9` but `7d pos60.1 28i 0c` collapse; RUM `70 portablessds +110 externalssds /28d` (~2.5+4/d); GA4 `0 affiliate_click` both; `src/pages/compare.astro:66` now `Portable SSDs Comparison (2026) — 78 Drives, Buying Guide & Prices` `66c` (was `Compared … Buying Guide & Prices` `53c`), `meta` front-loads `comparison & buying guide`, `H1` mirrors. `externalssds` branch untouched. Zero `href/rel/tag/linkCode` change — IndexNow `3367+3044 200` done.
> - **Day+3 2026-09-04 (Bing re-crawl check):** `node scripts/bing_pull_traffic.js` → target `portable ssds comparison` or `buying guide` CTR `0%→≥3%` or `portablessds /compare` page CTR `2.13%→≥3%` (`47i 1c` baseline). If impressions still flat (`30i/15i` unchanged) → `AMBIGUOUS` → single re-pull Day+4, no rollback yet (Bing lag `1-4d`).
> - **Day+7 2026-09-08 (GSC+GA4+RUM):** `gsc_pull_revenue.js --force` + `ga4_pull_traffic.js` + RUM `requestHost` pull → target GSC portablessds `/compare` `pos60.1 28i/7d 0c → pos≤35 ≥60i/7d ≥1c`; RUM portablessds `70/28d → ≥80/28d`; GA4 `0→≥1 affiliate_click` on portablessds; no regression `externalssds /compare pos34`. Write `RESULT (YYYY-MM-DD)` block to `AGENTS.md` closing the gate.
> - **Day+14 2026-09-15 close-out (full verdict):** all 4 scripts + RUM; if **all fail** (Bing still `0c`, GSC still `pos>50 0c`, RUM flat) → rollback `git revert 01ef4ab --no-edit` + PAT push per `AGENTS.md:Git Push Authentication` and document. If **any pass** → keep live, then data-gated H2H slug-map extension at `src/pages/compare/[slug].astro:106` only for URLs `≥10i/28d pos≤20 0c`. **Future AIs: keep this block + all START HERE blocks until their gates close; when new future pulls are determined, append them here — do not delete until done.**
...
> - **Server**: `src/pages/api/affiliate-click.ts` (POST, 204, validates + bounds all fields ≤1KB, inserts `{site_id, page_path, product_slug, retailer, cta_label}` into `affiliate_clicks`).
> - **Schema**: `affiliate_clicks` table + 3 indexes (`site_id+created_at`, `product_slug`, `page_path`) added to `db/schema.sql` AND already deployed to remote D1 via `--command=` (the `--file=` import endpoint 403s with this OAuth token — use `--command=`).
> - **Report**: `node scripts/affiliate_clicks_report.js` — prints 30d totals, 7d total, daily (14d), top pages, top products, retailer split. This is the ONLY per-URL attribution source: the Amazon Associates dashboard buckets the network's clicks as ASIN="others" (verified 2026-08-19: 19 clicks / 1 order / $239.99 / $6.00 earnings on `ssdnetwork07-20`, all bucketed "others").
> - **Verified live (2026-08-20)**: beacon present in DOM on both tenants (HTTP 200), POST → 204 both, rows landed in D1 (smoke rows deleted after verification — table currently 0 rows, waiting on real traffic).
> - **Compliance**: `git diff` shows zero changes to `href=`/`rel=`/`target=`/`linkCode=`/`tag=`/`data-affiliate=` (the only data-affiliate hit is the read-only `closest()` selector). Disclosure, CookieConsent, GeoAffiliateLink, affiliate.ts, sitemap — untouched. No URL surface change → **IndexNow NOT required**.
>
> **NEXT AGENT MUST**:
> 1. **Fold `node scripts/affiliate_clicks_report.js` into the 2026-08-23 Phase 1 gate pull** (Bing + GSC + GA4 + beacon report in one session). Cross-check the beacon's `clicks_7d`/`total_clicks` against the Associates dashboard "Clicks" figure (24-48h lag expected) — parity within ~±2 proves the attribution layer is truthful; the per-`page_path`/`product_slug` breakdown then becomes the input for Phase 2/3 targeting decisions (e.g. which H2H pair or product page actually converts).
> 2. **2026-08-25** (`6fdf14c` Day +7): include the beacon report — the home-page CTA funnel's `affiliate_click` count now comes from D1, not the consent-gated GA4 event.
> 3. **2026-08-30 (Phase 3 close-out)**: record a `RESULT (2026-08-30)` block noting the first per-page conversion data from `affiliate_clicks`.
> 4. If the beacon ever shows rows but the dashboard shows nothing (or vice versa), investigate the discrepancy before trusting either source — dashboard lag is the usual cause.
