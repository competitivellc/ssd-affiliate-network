...
// Falls back to a friendly "not configured" message when BING_WMT_API_KEY is unset.
...
async function checkKey() {
  ...
  if (!key) {
    console.log("BING_WMT_API_KEY not set. Skipping BWMT pull.");
...
async function run() {
  ...
  for (const domain of DOMAINS) {
    ...
    try {
      ...
      console.log(`28d TOTAL: clicks=${fmtNum(totalClicks)} impressions=${fmtNum(totalImpressions)} ctr=${fmtPct(avgCtr)}`);
      ...
      const topKws = [...kws]
        ...
        .slice(0, 20);
      if (topKws.length > 0) {
        console.log("\nTOP KEYWORDS (28d, sorted by impressions):");
        ...
        console.log("\n  (no keyword data returned for this window)");
      }
      ...
      try {
        ...
        if (topPages.length > 0) {
          console.log("\nTOP PAGES (28d, sorted by impressions):");
          for (const row of topPages) {
            const imp = Number(row.Impressions || 0);
            const clk = Number(row.Clicks || 0);
            const ctr = imp > 0 ? clk / imp : 0;
            const page = String(row.Page || "").slice(0, 80);
